        ...
        <Form ref="formModel"  class="form-block" :rules="servicesValidate" :model="formModel" :label-width="130">
          <Row>
            ...
            <Form-item label="身份证号码" prop="identitynumber">
               <Input @on-keyup="sha" v-model="formModel.identitynumber" style='width:400px' placeholder="请输入身份证号码" ></Input>
              <Button type="primary" @click.native="record('formModel')">查询居民信息</Button> 

              <!-- <Dropdown visible="true" trigger="custom" @on-click="xuanze" style="margin-left: 20px">
                <a href="javascript:void(0)">
                   <Input @on-keyup="sha" v-model="formModel.identitynumber" style='width:400px' placeholder="请输入身份证号码" ></Input> 
                </a>
                <Dropdown-menu  style='width:400px' slot="list">
                    <Dropdown-item name="1">驴打滚</Dropdown-item>
                    <Dropdown-item name="2">炸酱面</Dropdown-item>
                    <Dropdown-item name="3">豆汁儿</Dropdown-item>
                    <Dropdown-item name="4">冰糖葫芦</Dropdown-item>
                    <Dropdown-item name="5">北京烤鸭</Dropdown-item>
                </Dropdown-menu>
            </Dropdown> -->
    ...
    <template v-else-if="current === 1">
      <div style="height:80%;overflow:auto">
        <Form  ref="servicesModel" class="form-block" :model="servicesModel" :label-width="140">
          ...
          <Row>
            <Col span="12">
            <Form-item label="当前医疗机构" prop="institutionidentity">
              {{servicesModel.orgname}}
            </Form-item>
            </Col>
          </Row>
          <Row>
...
<script>
  // 接口
  import {
    addContact, getTeamsByCon, getFuWuByTeamsKey,
  ...
  export default {
    ...
    created: function () {
      ...
      // =========================================================查询当前登陆人的医疗机构，并且查询当前医疗机构的所有团队
      findCurrentOrg(para).then((res) => {
        if (res.status === 200) {
          console.log(res.data)
          this.servicesModel.institutionidentity = res.data.id
          this.servicesModel.orgname = res.data.orgname
          // 根据当前医疗机构的ID查询当前医疗机构的团队
          this.applyDoctorTeam = []
          let para = {
            institutionidentity: this.servicesModel.institutionidentity
          }
          // 根据医疗机构ID查询一声团队信息
    ...
    methods: {
      ...
      sha () {
        this.xianshi = true
        // alert(this.formModel.identitynumber)
      },
      // ·················································································选择医生团队
      yishengclick () {
      ...
      assignment () {
        ...
        // 当前机构ID  institutionidentity
        this.insertModel.institutionidentity = this.servicesModel.institutionidentity
        // 当前机构名称 institutionname
        this.insertModel.institutionname = this.servicesModel.orgname
