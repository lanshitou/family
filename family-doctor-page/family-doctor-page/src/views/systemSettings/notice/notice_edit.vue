                    ...
                    <Row type="flex" justify="start">
                        ...
                        <Form-item label="发布方式" prop="publishmode">
                          ...
                          <span v-show="sendTime" style="float:left">
                            <Date-picker type="datetime"  placeholder="请选择发布日期" style="width: 200px" @on-change="changeTime" :editable=false v-model="noticeModel.publishtime"></Date-picker>
                          </span>
    ...
    doSubmit (noticeModel) {
      ...
      if (boo) {
        this.$refs['noticeModel'].validate((valid) => {
          if (valid) {
            updateNotices(this.noticeModel).then((res) => {
              if (res.status === 200) {
