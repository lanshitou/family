                ...
                <Form ref="noticeModel" class="form-block" :model="noticeModel" :label-width="100" :rules="noticeValidate">
                    <Row type="flex" justify="start">
                        ...
                        <Form-item label="主题" prop="title">
                            <Input v-model="noticeModel.title"   placeholder="请输入主题" style="width: 400px"></Input>
                        </Form-item>
                        </Col>
                    </Row>
                    <Row type="flex" justify="start">
                    ...
                    <Row type="flex" justify="start">
                        ...
                        <Form-item label="发布方式" prop="publishmode">
                          ...
                          <span v-show="sendTime" style="float:left">
                            <Date-picker  type="datetime"  placeholder="请选择发布日期" style="width: 200px" @on-change="changeTime" :editable=false></Date-picker>
                          </span>
                    ...
                    <Row type="flex" justify="start">
                        ...
                        <Form-item label="发布内容" prop="content">
                            <Input v-model="noticeModel.content" type="textarea" :autosize="{minRows: 6}" size="large"></Input>
                        </Form-item>
                        </Col>
                    </Row>
                    <Row> 
  ...
  methods: {
    doSubmit (noticeModel) {
      ...
      if (boo) {
        this.$refs['noticeModel'].validate((valid) => {
          if (valid) {
            insertNotices(this.noticeModel).then((res) => {
              if (res.status === 200) {
    ...
    changeTime (change) {
      ...
      if (date < nowDate) {
        ...
        this.noticeModel.publishtime = this.formatDate(date)
      } else {
        this.noticeModel.publishtime = this.formatDate(date)
        this.showNotice = ''
      }
