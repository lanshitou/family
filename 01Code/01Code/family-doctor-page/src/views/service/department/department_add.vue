  ...
  data () {
    return {
      ...
      inquiryValidate: {
        ...
        { type: 'string', max: 50, message: '科室名称不能多于50个字符', trigger: 'blur' }
        ],
        patientskeyword: [
        ...
        { type: 'string', max: 50, message: '年龄条件不能多于50个字符', trigger: 'blur' }
        ]
      }
    }
  },
  methods: {
    ...
    saveform (formData) {
      this.$refs['inquiryModel'].validate((valid) => {
        if (valid) {
          insertOffices(this.inquiryModel).then((res) => {
            if (res.status === 200) {
