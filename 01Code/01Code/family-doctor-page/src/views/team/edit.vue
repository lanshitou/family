    ...
    <div style="margin:2px">
      <div class="layout-content-main">
        <div style="background:#f3f3f3;padding:2px;text-align:right">
          <Button-group>
            <Button type="primary" style="margin:1px" @click="add">新增成员</Button>
        ...
        <Modal v-model="addModel" title="团队成员添加"  class="update-model" @on-ok="addTeamMember">
          <Form :label-width="80" ref="addData" :model="addData" :rules="formValidate">
            ...
            <Form-item label="角色职责" prop="jobdescription">
              <Input v-model="addData.jobdescription" style="width: 200px" readonly></Input>
            </Form-item>
          </Form>
        </Modal>
        <!--团队成员编辑弹出框-->
        <Modal v-model="updateModel" title="团队成员编辑" class="update-model" @on-ok="saveTeamMember">
          <Form :label-width="80">
            ...
            <Form-item label="角色">
              <Select v-model="updateData.doctorroleidentity" placeholder="请选择角色名称" @on-change="changeUR" :label-in-value="true">
...
<script>
import {
  getTeamsMember, updateTeamsMember, getInstitutionByUser, getAllRoles, getAllDocotr, getTeamsByCon, insertTeamsMember, delTeamsMember
} from '../../api/api'
export default {
  data () {
    return {
  ...
  created: function () {
    ...
    // 根据条件查询团队信息
    getTeamsByCon(data).then((res) => {
      this.addData.medicalteamidentity = this.teamId
      this.addData.medicalteamname = res.data[0].teamname
    })
    // 获得当前医疗机构
    getInstitutionByUser().then((res) => {
      this.institutionList = res.data
      var institutionidentity = res.data.id
      let para = {
        institutionidentity: institutionidentity
      }
      // 根据医疗机构ID查询医生信息
      getAllDocotr(para).then((res) => {
  ...
  methods: {
    ...
    update (index) {
      ...
      this.updateData.doctoridentity = this.teamList[index].doctoridentity
