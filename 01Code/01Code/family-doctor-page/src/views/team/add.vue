...
<script>
import {
  getAllRoles, getAllDocotr, insertTeam, findCurrentOrg
} from '../../api/api'
export default {
  data () {
    return {
  ...
  created: function () {
    ...
    findCurrentOrg().then((res) => {
      let para = {
        institutionidentity: res.data.institutionidentity
      }
    // 获得当前医疗机构下的所有签约医生
      getAllDocotr(para).then((res) => {
        this.teammember = res.data
      })
      this.addForm.institutionidentity = res.data.institutionidentity
      this.addForm.institutionname = res.data.institutionname
    })
    // 查询全部医疗团队角色信息
